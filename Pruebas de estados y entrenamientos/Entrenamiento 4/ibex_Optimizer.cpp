...
namespace ibex
{
	...
	Optimizer::Status Optimizer::optimize()
	{
		...
		try
		{
			...
			// REGISTRO SILENCIOSO (Solo si estamos en fase de diagnóstico)
			//std::ofstream log_file("state_trace.csv", std::ios::app);
			//if (log_file.is_open()) {
				// Guardamos: Problema, Estado, d1, d2, d3, d4, AcciónTomada
				//log_file << last_state_index << "," 
						//<< (last_state_index % 3) << ","           // d1
						//<< ((last_state_index / 3) % 3) << ","     // d2
						//<< ((last_state_index / 9) % 2) << ","     // d3
						//<< ((last_state_index / 18) % 2) << ","    // d4
						//<< last_action_taken << "\n";
				//log_file.close();
			//}
