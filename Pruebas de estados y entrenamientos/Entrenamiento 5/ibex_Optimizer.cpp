...
void signal_handler(int signum) {
    if (global_optimizer_ptr) {
		...
		global_optimizer_ptr->save_state_distribution();
    }
    std::exit(signum);
}
...
namespace ibex
{
												   ...
												   time(0), nb_cells(0), cov(NULL), clustering_params()
	{
		...
		// 2. Cargar el estado de la Temperatura (Tau) en lugar de Epsilon
		if (!exploitation_mode) {
			std::ifstream tfile("tau_state.txt"); // Cambiamos el nombre del archivo
			if (tfile.is_open()) {
				tfile >> tau; 
				tfile.close();
			} else {
			...
			tau = TAU_MIN; // En modo evaluación usamos la temperatura mínima
		}
	...
	void Optimizer::start(const IntervalVector &init_box, double obj_init_bound)
	{
        ...
        std::cout << "[SESIÓN RL] Tau actual: " << std::fixed << std::setprecision(6) << tau << std::endl;
	...
	Optimizer::Status Optimizer::optimize()
	{
		...
		try
		{
			...
			steps_since_last_restart++;
			//if (steps_since_last_restart % 50 == 0) {
				//cout << "DEBUG: Pasos acumulados: " << steps_since_last_restart << " | Estado actual: " << get_discrete_state() << endl;
			//}
			...
			state_frequency[last_state_index]++; // sirve para ver frecuencia de estados


            // 2. [DECISIÓN DE ACCIÓN (POLÍTICA SOFTMAX)]
			double current_tau = (exploitation_mode) ? TAU_MIN : tau;

			// Obtenemos los valores Q del estado actual
			double q_cont = Q_Table[last_state_index][0];
			double q_rein = Q_Table[last_state_index][1];

			// TRUCO DE ESTABILIDAD: Restamos el máximo para evitar overflows con exp()
			double max_q = std::max(q_cont, q_rein);
			double exp_cont = std::exp((q_cont - max_q) / current_tau);
			double exp_rein = std::exp((q_rein - max_q) / current_tau);
			double sum_exp = exp_cont + exp_rein;

			// Calculamos la probabilidad de elegir la Acción 1 (Reiniciar)
			double prob_rein = exp_rein / sum_exp;

			if (exploitation_mode) {
				// En explotación pura, elegimos simplemente el mejor
				last_action_taken = (q_rein > q_cont) ? 1 : 0;
			} else {
				// En entrenamiento, usamos la probabilidad Softmax
				if ((double)rand() / RAND_MAX < prob_rein) {
            ...
            if (last_action_taken == 1) { // ACCIÓN: REINICIAR
                ...
                double time_penalty = (timer.get_time() - cpu_time_before_step) * (-20.0); // Escalado a 10x
                ...
                if (loup < loup_before_step) {
					...
					R_progreso += 5.0 * std::log1p(delta * 1e6); // log1p(x) es log(1+x)
				}
				
				R_progreso += 5.0 * last_noise_ratio; // <--- BONO POR LIMPIAR EL ÁRBOL
            ...
            try {
				...
				double R_costo = time_consumed * (-20.0);
				...
				catch (NoBisectableVariableException &)
				{
					...
					// 2. Aprendizaje RL (solo si no estamos en modo explotación)
					if (!exploitation_mode && last_state_index != -1) {
						double time_penalty = (timer.get_time() - cpu_time_before_step) * (-10.0);
						double R_fail = -20.0;
						QL_update(time_penalty + R_fail);
						
						// Actualizar métricas internas
						poda_window.push_back(false);
						if (poda_window.size() > PODA_WINDOW_SIZE) poda_window.pop_front();
						last_step_loup_improved = false;
					}
			...
			// ───────────────────────────────────────────────────────────────────
			// DECAY DE TAU (UNA VEZ POR EPISODIO)
			// ───────────────────────────────────────────────────────────────────
			if (!exploitation_mode) {
				double old_tau = tau;
				...
				if (trace >= 0) {
					std::cout << "\n[QL_DECAY] Tau actualizado: " 
							<< old_tau << " → " << tau << std::endl;
				}
			}
			...
			if (!exploitation_mode) { // Solo si estábamos en modo entrenamiento
				...
				// NUEVO: Guardar el valor de tau actual para la siguiente ejecución
				std::ofstream tfile("tau_state.txt");
				if (tfile.is_open()) {
					tfile << tau; 
					tfile.close();
					if (trace >= 1) std::cout << "[QL_GUARDADO] Tau guardado: " << tau << std::endl;
				}
			}
		...
		{
			status = TIME_OUT;
			// Si estamos entrenando, aplicamos un castigo severo al último estado
			if (!exploitation_mode && last_state_index != -1) {
        		QL_update(-1000.0); 
    }
		}
    ...
    void Optimizer::cluster_restart(Timer& timer)
    {
        ...
        if (clustering_params.choice == ClusteringParams::Algorithm::DBSCAN)
        {
            ...
            if (trace)
            {
                ...
                if (N > 0)
                {
                    int noise_count = 0;
                    for (size_t i = 0; i < N; ++i){
                        if (result_labels[i] == DBSCAN_NOISE) noise_count++;
					}
					this->last_noise_ratio = (N > 0) ? (double)noise_count / N : 0.0;	
                    if (noise_count > 0)
     ...
     // --- PROCESAMIENTO DE CLÚSTERES Y CREACIÓN DE HULLS  ---
		...
		steps_since_last_restart = 0; // Resetear contador tras reiniciar
	...
	int d2;
	if (log_size < 5.3) { //Bajo: < 200 nodos
		d2 = 0;
	} else if (log_size < 9.2) {  // Medio: 50 - 10000 nodos
		d2 = 1;
	} else { // Alto: > 10000 nodos
		d2 = 2;
	}
	...
	int d3;
    if (steps_since_last_restart > 100) {
        d3 = 0; // Se le da una segunda oportunidad al agente
    } else {
        d3 = (S3_raw < 0.15) ? 0 : 1; // Si es 0 va bien, si es 1 va mal.
    }
	...
	if (!poda_window.empty()) {
		...
		d4 = (tasa_poda >= 0.05) ? 1 : 0; // 20% threshold. Si es 1 va bien, si es 0 va mal.
	} else {
	...
	if (!std::isfinite(reward)) reward = -1.0;
	...
	// Si el valor futuro es corrupto, lo ignoramos
    if (!std::isfinite(max_Q_next)) max_Q_next = 0.0;

    double target = reward + QL_GAMMA * max_Q_next;
    double td_error = target - Q_Table[last_state_index][last_action_taken];
    // Actualización con protección
...
void Optimizer::save_state_distribution() {
    // Leer variables de entorno pasadas por el script .sh
    const char* problem = std::getenv("IBEX_PROB_NAME");
    const char* seed    = std::getenv("IBEX_SEED");
    
    std::ofstream ffile("state_distribution.csv", std::ios::app);
    if (ffile.is_open()) {
        // Formato: Problema, Semilla, S00, S01, ..., S35
        ffile << (problem ? problem : "unknown") << "," 
              << (seed ? seed : "0") << ",";
        
        for (int i = 0; i < 36; ++i) {
            ffile << state_frequency[i] << (i == 35 ? "" : ",");
        }
        ffile << "\n";
        ffile.close();
    }
