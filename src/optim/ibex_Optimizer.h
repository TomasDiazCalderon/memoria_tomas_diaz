...
public:
	void save_state_distribution();

	void save_ql_data(); // Método para guardar Q-Table y Epsilon
...
private:
    ...
    int restart_threshold = 40;       ///< iteraciones sin mejora antes de reiniciar
    ...
    void cluster_restart(ibex::Timer& timer);           ///< prototipo de la rutina de reinicio


	// ── IMPLEMENTACIÓN Q-LEARNING (Prototipo Tabular) ──
    
    // [1] Hiperparámetros de Q-Learning (Constantes)
    const double QL_ALPHA = 0.1;   // Tasa de aprendizaje
    const double QL_GAMMA = 0.99;  // Factor de descuento

    // [2] Variable de Exploración (epsilon) y su decremento
    double epsilon = 1.0;         // Tasa de exploración (inicia alta)
	double tau;
    const double EPSILON_DECAY = 0.991; // Factor de decaimiento (Ajustar)
    const double EPSILON_MIN = 0.00;     // Valor mínimo de epsilon
	bool exploitation_mode = true; // Si es TRUE, epsilon se fija a 0.0 (explota)
	const double TAU_INITIAL = 10.0; // Temperatura alta = mucha exploración
	const double TAU_MIN     = 0.1;  // Temperatura baja = casi explotación pura
	const double TAU_DECAY   = 0.986; // Factor de enfriamiento
	

    // [3] Q-TABLE: 36 estados (Index 0-35) x 2 acciones (0:Continuar, 1:Reiniciar)
    double Q_Table[36][2] = {{0.0}};
	int state_frequency[36] = {0}; 
	int steps_since_last_restart = 0; // Contador para el "perdón" de calidad
	
    // [4] Variables Auxiliares para el Cálculo de Recompensa y Actualización
    int last_state_index = 0;
    int last_action_taken = 0;
    double loup_before_step = POS_INFINITY;
    double cpu_time_before_step = 0.0;
	double last_rejection_rate = 0.0; // Añadir: Almacena la Tasa de Hulls Rechazados (S3)
	double last_noise_ratio;
	bool last_step_loup_improved = false; // Flag para S4

	std::deque<bool> poda_window; // Ventana de las últimas N mejoras
    const int PODA_WINDOW_SIZE = 100; // Tamaño de la ventana
    
    // [5] Funciones de Control RL
    int get_discrete_state();   // Mapea las 4 métricas a un índice [0-35]
    void QL_update(double reward, int forced_next_state = -1); // Aplica la Ecuación de Bellman
	void load_q_table();
